...
The analysis is implemented with with the worklist algorithm. The analysis attaches an $(int,boolean)$ tuple to every meaningful node. A node is considered meaningful if the node has an integer-mode. We will reference the first value as \emph{stable bits} and the second value as \emph{is positive}. \newline
